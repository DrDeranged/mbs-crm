...
|---|---|---|
| A — Mapper and persistence model | PASS | Exact 28-column mapping and normalization: `artifacts/api-server/src/lib/intake/usfa.ts:152-225`; exhaustive mapper tests: `artifacts/api-server/src/lib/intake/usfa.test.ts`; source-only migration: `lib/db/migrations/025_usfa_intake.sql:1-32`. The existing production `companies` table is modeled by `lib/db/src/schema/companies.ts`; no companies table is created. SSN/DOB are only in the encrypted lead-keyed prefill, never the lead row. |
...
| E — Dormant webhook | PASS | Strict payload and raw-body HMAC: `artifacts/api-server/src/routes/usfaIntake.ts:9-101`; exact signature tests: `artifacts/api-server/src/routes/usfaIntake.test.ts`; enable flag defaults false in migration 026 and settings. |
...
- External ID is checked first. Email/phone matches attach a re-application to
  the existing lead while updating its encrypted lead-keyed prefill, notes, statement
...
The development database was rechecked read-only with:
...
It returned the existing production-shaped columns:
`id, lead_id, name, address, city, state, zip, industry,
time_in_business_months, annual_revenue, created_at, updated_at`.
`lib/db/src/schema/companies.ts` already matches that table. No `companies`
table is created by a USFA migration.

Migration responsibilities are now explicit:

- `025_usfa_intake.sql`: `usfa_intake_log`, lead-keyed encrypted
  `usfa_intake_prefill`, nullable `leads.external_id` with a unique index, and
  raw credit/revenue band columns on `leads`.
- `026_usfa_intake_runtime_support.sql`: settings flags, nullable task queue,
  Gmail receipt log, document label, and secure prefill invite support.

```text
pnpm -r --if-present test
  API: 234 passed, 0 failed
...
2. Apply migrations 025 and 026 through the approved migration process before
   starting either poller or enabling the webhook.
