...
export function LenderSubmissionsPanel({ leadId, dealId, onStageMove }: Props) {
  ...
  const [form, setForm] = useState({ lender: "", viaBroker: "", endLender: "", date: new Date().toISOString().slice(0, 10), status: "submitted", notes: "" });
  ...
  const saveManual = async () => {
    ...
    createManual.mutate({ id: leadId, data: { lender_id: Number(form.lender), via_broker_id: form.viaBroker ? Number(form.viaBroker) : undefined, end_lender_id: form.endLender ? Number(form.endLender) : undefined, deal_id: dealId, submitted_at: new Date(`${form.date}T12:00:00`).toISOString(), status: form.status as any, notes: form.notes || null, approval_pdf_base64 } } as any, { onSuccess: () => { refresh(); setOpen(false); setPdf(null); }, onError: () => undefined });
  };
  ...
  return <section className="space-y-3 rounded-lg border bg-white p-4" data-testid="lender-submissions-panel">
    ...
    {submissions.map((s) => <div key={s.id} className="grid gap-2 border-t pt-2 text-sm md:grid-cols-[1fr_auto]"><div><div className="flex items-center gap-2"><b>{s.lender?.name ?? `Partner #${s.lenderId}`}</b>{s.lender?.partnerType && <Badge variant="outline">{s.lender.partnerType.replace("_", " ")}</Badge>}</div><div className="text-xs text-muted-foreground">Date submitted: {format(new Date(s.sentAt), "MMM d, yyyy")} · Source: {s.source ?? "crm"}</div>{s.viaBrokerId && <div className="text-xs text-muted-foreground">Via broker #{s.viaBrokerId}{s.endLenderId ? ` · placed with #${s.endLenderId}` : ""}</div>}<div className="text-xs text-muted-foreground">Decision date: {s.decisionDate ? format(new Date(s.decisionDate), "MMM d, yyyy") : "—"}</div><div className="flex gap-3">{s.hasApprovalAttachment && <Button type="button" variant="link" className="h-auto p-0 text-xs" onClick={() => void downloadApproval(s)}>Download approval attachment</Button>}{s.hasExactPackage && <Button type="button" variant="link" className="h-auto p-0 text-xs" onClick={() => void downloadPackage(s)}>Download sent package</Button>}</div><Input className="mt-1 h-7 text-xs" defaultValue={s.notes ?? ""} placeholder="Notes" onBlur={(e) => { if (e.target.value !== (s.notes ?? "")) update.mutate({ id: s.id, data: { notes: e.target.value } }, { onSuccess: refresh }); }} /></div><Select value={s.status} onValueChange={(v) => changeStatus(s, v)}><SelectTrigger className={`h-7 w-28 text-xs ${colors[s.status] ?? ""}`}><SelectValue /></SelectTrigger><SelectContent>{["submitted", "approved", "declined", "funded", "withdrawn"].map((v) => <SelectItem key={v} value={v}>{v}</SelectItem>)}</SelectContent></Select></div>)}
    <Dialog open={open} onOpenChange={setOpen}><DialogContent><DialogHeader><DialogTitle>Log partner submission</DialogTitle></DialogHeader><div className="space-y-3">{(lendersQuery.isError || lenderList.malformed) && <InlineListError title="Couldn’t load partners" status={lendersQuery.isError ? getQueryErrorStatus(lendersQuery.error) : 200} detail={lenderList.malformed ? "The server returned an unexpected partner response." : undefined} onRetry={() => void lendersQuery.refetch()} />}<Label>Partner</Label><Select value={form.lender} onValueChange={(v) => setForm({ ...form, lender: v })}><SelectTrigger><SelectValue placeholder="Select partner" /></SelectTrigger><SelectContent>{lenders.filter((l: any) => l.partnerType !== "broker_in").map((l: any) => <SelectItem key={l.id} value={String(l.id)}><span className="flex items-center gap-2">{l.name}<Badge variant="outline">{(l.partnerType ?? "direct_lender").replace("_", " ")}</Badge></span></SelectItem>)}</SelectContent></Select>{lenders.find((l: any) => String(l.id) === form.lender)?.partnerType === "broker_out" && <><Label>End lender (optional)</Label><Select value={form.endLender} onValueChange={(v) => setForm({ ...form, endLender: v })}><SelectTrigger><SelectValue placeholder="Log later or select now" /></SelectTrigger><SelectContent>{lenders.filter((l: any) => l.partnerType === "direct_lender").map((l: any) => <SelectItem key={l.id} value={String(l.id)}>{l.name}</SelectItem>)}</SelectContent></Select></>}<Label>Date submitted</Label><Input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} /><Label>Status</Label><Select value={form.status} onValueChange={(v) => setForm({ ...form, status: v })}><SelectTrigger /><SelectContent>{["submitted", "approved", "declined", "funded", "withdrawn"].map((v) => <SelectItem key={v} value={v}>{v}</SelectItem>)}</SelectContent></Select><Label>Notes</Label><Input value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} /><Label>Approval PDF (optional)</Label><Input type="file" accept="application/pdf" onChange={(e) => setPdf(e.target.files?.[0] ?? null)} /><Button disabled={!form.lender || createManual.isPending} onClick={saveManual}>{createManual.isPending ? "Saving…" : "Save submission"}</Button></div></DialogContent></Dialog>
  </section>;
}