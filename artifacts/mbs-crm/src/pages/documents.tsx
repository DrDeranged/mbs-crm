...
export default function Documents() {
  ...
  const load = () => fetch(`${api}/collateral/templates?includeDrafts=${admin}`, { credentials: "include" }).then(r => r.json()).then(setTemplates);
  useEffect(() => { void load(); }, [admin]);
  useEffect(() => { if (admin) fetch(`${api}/users?role=rep&isActive=true`, { credentials: "include" }).then(r => r.json()).then(x => setReps(x.users ?? x)); }, [admin]);
  useEffect(() => {
  ...
  const leads = (leadData as any)?.leads ?? (leadData as any)?.items ?? [];
