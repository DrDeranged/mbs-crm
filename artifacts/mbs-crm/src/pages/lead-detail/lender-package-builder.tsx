...
export function LenderPackageBuilderDialog({ leadId, open, onOpenChange, submitMode = false }: {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
    ...
    <div className="grid gap-2 sm:grid-cols-3"><label className="flex items-center justify-between rounded border p-2 text-sm">Mask SSN <Switch checked={config.options.maskSsn} disabled={me?.role !== "admin"} onCheckedChange={(maskSsn) => setConfig({ ...config, options: { ...config.options, maskSsn } })} /></label><label className="flex items-center justify-between rounded border p-2 text-sm">Cover page <Switch checked={config.options.includeCoverPage} onCheckedChange={(includeCoverPage) => setConfig({ ...config, options: { ...config.options, includeCoverPage } })} /></label><label className="flex items-center justify-between rounded border p-2 text-sm">Footer each page <Switch checked={config.options.includeFooter} onCheckedChange={(includeFooter) => setConfig({ ...config, options: { ...config.options, includeFooter } })} /></label></div>
